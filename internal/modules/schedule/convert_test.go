...
func TestParseStream(t *testing.T) {
	cases := []struct {
		...
		{"", "", []string{}},
		// Языковой поток: базовые группы из хвоста имени, поток остаётся.
		{"", "006073_2 Иностранный язык (КАЯиПК)-10 СОЦ25-6_7", []string{"006073_2 Иностранный язык (КАЯиПК)-10 СОЦ25-6_7", "СОЦ25-6", "СОЦ25-7"}},
		{"", "006903_1 Иностранный язык (КАЯиПК)-1 СОвПиБ26-1", []string{"006903_1 Иностранный язык (КАЯиПК)-1 СОвПиБ26-1", "СОвПиБ26-1"}},
		{"", "006839_1 Иностранный язык (КИЯиМК)-3 Русский язык, межфак  (РКИ26-15)", []string{"006839_1 Иностранный язык (КИЯиМК)-3 Русский язык", "межфак  (РКИ26-15)", "РКИ26-15"}},
		// Поток без базовых групп — как был.
		{"", "006892_1 Иностранный язык (КАЯиПК)-17", []string{"006892_1 Иностранный язык (КАЯиПК)-17"}},
	}
	for _, c := range cases {
...
func TestPodgruppaIzImeniPotoka(t *testing.T) {
	if got := subgroupOf("", "", "006073_2 Иностранный язык (КАЯиПК)-10 СОЦ25-6_7"); got != "подгруппа 10" {
		t.Errorf("получили %q", got)
	}
	if got := subgroupOf("2", "", "006073_2 Иностранный язык (КАЯиПК)-10 СОЦ25-6_7"); got != "2" {
		t.Errorf("явное поле важнее имени, получили %q", got)
	}
	if got := subgroupOf("", "ПИ24-1; ПИ24-2", ""); got != "" {
		t.Errorf("у обычного потока подгруппы нет, получили %q", got)
	}
}
