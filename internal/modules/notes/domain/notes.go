...
// LLMCall — один вызов модели конспекта: сколько потратили и чем кончилось.
//
// Остатка лимита bigmodel.cn не сообщает ни в заголовках, ни отдельным
// методом, поэтому расход считается у нас, по каждому вызову. Отказ
// хранится с кодом провайдера: «баланс», «частота» и «дневной лимит» —
// разные беды, и чинятся они по-разному.
type LLMCall struct {
	At               time.Time
	Kind             string // summary | part | probe
	Model            string
	OK               bool
	PromptTokens     int
	CompletionTokens int
	Duration         time.Duration
	Code             string // код отказа провайдера, «1113»; пусто — не отказ провайдера
	Message          string
}

// LLMHint — что значит код отказа bigmodel.cn. Пусто — код неизвестен, и
// объяснение не выдумывается.
func LLMHint(code string) string {
	switch code {
	case "1000", "1001", "1002", "1003", "1004":
		return "ключ API недействителен или просрочен — нужен новый в кабинете bigmodel.cn"
	case "1113":
		return "баланс исчерпан или нет пакета ресурсов — пополнить в кабинете bigmodel.cn"
	case "1211":
		return "модели с таким именем нет — её сняли с раздачи, нужна другая в конфиге"
	case "1261":
		return "текст длиннее окна модели"
	case "1301":
		return "ответ заблокирован фильтром содержимого провайдера"
	case "1302", "1303", "1305":
		return "превышена частота запросов — провайдер просит подождать"
	case "1304":
		return "исчерпан дневной лимит вызовов — сбросится завтра"
	}
	return ""
}
