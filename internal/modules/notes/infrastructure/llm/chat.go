...
func finalPrompt(in notes.SummaryInput, text string) string {
	...
	b.WriteString(".")
	if len(in.Gaps) > 0 {
		// Без этого модель сошьёт края пропуска в одну мысль и досочинит
		// связку, которой на паре не было.
		b.WriteString("\n\nЗапись прерывалась — телефон выключал микрофон: ")
		for i, g := range in.Gaps {
			if i > 0 {
				b.WriteString("; ")
			}
			b.WriteString(g.Label())
		}
		b.WriteString(". В расшифровке эти места отмечены «[пропуск в записи ~N мин]». Это не слова преподавателя. Не додумывай пропущенное: в конспекте на этом месте поставь отдельный абзац «Пропуск в записи, около N мин» и продолжай с того, что прозвучало после.")
	}
